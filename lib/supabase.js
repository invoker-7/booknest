...
async function selectProducts(build) {
  ...
  let res = await build(db.from("books").select(WITH_SHOP));
  if (res.error) res = await build(db.from("books").select("*"));
  return res;
}

/** ดึงรายการสินค้าดิจิทัลที่เปิดขายสำหรับหน้าร้าน (เก็บในตาราง books) */
export async function getBooks({ shopId } = {}) {
  if (!isSupabaseConfigured) return [];
  try {
    const { data, error } = await selectProducts((q) => {
      q = q.order("sort", { ascending: true });
      return shopId ? q.eq("shop_id", shopId) : q;
    });
    if (error) {
...
export async function getBook(id) {
  ...
  try {
    const { data, error } = await selectProducts((q) => q.eq("id", id).maybeSingle());
    if (error) {
...
export async function getShop(id) {
  ...
  try {
    ...
    console.error("getShop: Supabase is unreachable:", error.message);
    return null;
  }
}
